...
public class OtherFollowingActivity extends AppCompatActivity implements OtherFollowingAdapter.OnItemClickListener {

    private RecyclerView recyclerView;
    private List<FollowingData4> mFollows;
    private OtherFollowingAdapter fAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //deletebutton = (Button) findViewById(R.id.fdelete);

        database = FirebaseDatabase.getInstance();
        followreference = database.getReference("MyPage").child(id1).child("팔로잉");

        mFollows = new ArrayList<>();

        recyclerView = (RecyclerView) findViewById(R.id.msgLv);

        fAdapter = new OtherFollowingAdapter(mFollows);
        fAdapter.setOnItemClickListener(OtherFollowingActivity.this);
    ...
    public void onWhatEverClick(int position) {
        ...
        if(id.equals(id1)){
            ...
            Intent intent = new Intent(OtherFollowingActivity.this, MypageActivity.class);
            intent.putExtra("nickname", nickname);
            intent.putExtra("id", id);
            startActivity(intent);
        }
        else if (!id1.equals(id)){
            Intent intent = new Intent(OtherFollowingActivity.this, OthersPageActivity.class);
            intent.putExtra("nickname", nickname);
